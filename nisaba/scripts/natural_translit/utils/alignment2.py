...
class Alignment(exp.Expression):
  ...
  Attributes:
    ...
    priority: Priority of the alignment. When the applied cost of multiple
      alignments are equal, the rule with the highest priority will be applied.
    applied_cost: Cost of the alignment when it's applied in context.
    source: Source of the alignment.

  Alignment sources:
    ALIGNER: Alignments from an aligner output that doesn't correspond to an
      Alignable from other sources. Eg. identity or token boundary alignments.
    ...
    LEXICON: Alignables that come from a lexicon that will be prioritised over
      other Alignables. Eg. frequent affixes or high profile entity names.
  ...
  def _side_str(self, side: exp.Expression) -> str:
    if side.is_any():
      return str(side)
    return side.item(0).text if len(side) == 1 else str(side)
...
Alignment.ERROR = Alignment.constant(
    'error', exp.Atomic.CTRL.nor, exp.Atomic.CTRL.nor, op.Operation.COMMON.error
)
